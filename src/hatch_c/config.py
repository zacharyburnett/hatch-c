...
from sys import base_exec_prefix, exec_prefix, executable
...
class HatchCExtension(BaseModel):
    name: str
    sources: List[str]
...
class HatchCBuildConfig(BaseModel):
    ...
    platform: Optional[HatchCPlatform] = Field(default_factory=HatchCPlatform.default)

    @model_validator(mode="wrap")
    @classmethod
    def validate_model(cls, data, handler):
        if "toolchain" in data:
            data["platform"] = HatchCPlatform.platform_for_toolchain(data["toolchain"])
            data.pop("toolchain")
...
class HatchCBuildPlan(HatchCBuildConfig):
    ...
    def generate(self):
        ...
        for extension in self.extensions:
            compile_flags = self.platform.compile_flags(extension, self.build_type)
            link_flags = self.platform.link_flags(extension, self.build_type)
